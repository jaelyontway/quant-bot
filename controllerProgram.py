...
while True:
    ...
    if os.path.isfile(news_src) and os.path.isfile(price_src):
        ...
        # Copy output.txt from src/east/ to simulation/
        output_src = os.path.join(os.getcwd(), "src", "east", "output.txt")
        output_dest = os.path.join(simulation_folder, "output.txt")
        if os.path.isfile(output_src):
            shutil.copy(output_src, output_dest)
            print("output.txt copied from src/east/ to simulation/")
        else:
            print("Warning: output.txt not found in src/east/")
