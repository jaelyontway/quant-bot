...
def _merge_config(base: AppConfig, overrides: dict) -> AppConfig:
    ...
    cfg["default_hours_after"] = defaults_section.get(
        "hours_after", cfg["default_hours_after"]
    )
    ...
    cfg["trading_days"] = defaults_section.get(
...
def fetch_nvda_prices_around_event(
    ...
    raw_bars, query_meta = _query_stock_api(
        event_time_local, end_time_local, interval_minutes
    )
    ...
    end_time = time(16, 0)
    in_regular_hours = local_ts.dt.time.between(
...
def fetch_prices_from_day(
    target_date: str, interval_minutes: int,
) -> tuple[pd.DataFrame, dict]:
    market_tz = ZoneInfo(CONFIG.market_timezone)
    ...
    prices, metadata = fetch_nvda_prices_around_event(
        anchor_ts, hours_span, interval_minutes
    )
...
def main() -> None:
    ...
    news_df, domain_counts = fetch_news_for_date(
        target_date, keywords, news_timezone
    )
    ...
    matched_domains = sorted(
        [domain for domain, count in domain_counts.items() if count]
    )
    missing_domains = sorted(
        [domain for domain, count in domain_counts.items() if not count]
    )
    print(
        f"Domains with matches: "
        f"{', '.join(matched_domains) if matched_domains else 'none'}"
    )
    print(
        f"Domains without matches: "
        f"{', '.join(missing_domains) if missing_domains else 'none'}"
    )

    combined_prices, price_metadata = fetch_prices_from_day(
        target_date=target_date, interval_minutes=interval_minutes
    )
    ...
    if price_metadata:
        ...
        print(
            f"Price fetch metadata saved to {metadata_path} for verification."
        )
